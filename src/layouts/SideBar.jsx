...
const SideBar = () => {
  ...
  return (
    <div className="flex">
      {/* Sidebar */}
      {!isMobile && (
        <aside className={`fixed top-0 left-0 z-40 h-screen transition-all border-r border-gray-200 bg-[#4264D0] text-white ${isSidebarOpen ? 'w-55' : 'w-20'}`}>
          <div className="flex items-center m-2">
            ...
            <button 
              ...
              className={`p-[10px] w-[30px] h-[30px] flex items-center transition-all 
                          ${isSidebarOpen ? 'ml-[138px] bg-close rounded-4xl' : 'ml-[-2px] bg-close rounded-full'}`}
            >
          ...
          <ul className="space-y-2 font-medium">
            <NavLink to="/dasboard-admin" className={({ isActive }) => `flex items-center py-4 m-2 px-2 rounded-sm ${isActive ? 'bg-[#718ADE]' : 'hover-side'}`}>
              <Home className={`w-7 h-7 flex ${isSidebarOpen ? 'ml-[10px]' : 'mx-auto'}`} />
              {isSidebarOpen && <span className="ms-3">Dashboard</span>}
            </NavLink>
            <NavLink to="/pendataan-barang" className={({ isActive }) => `flex items-center py-4 m-2 px-2 rounded-sm ${isActive ? 'bg-[#718ADE]' : 'hover-side'}`}>
              <Package className={`w-7 h-7 flex ${isSidebarOpen ? 'ml-[10px]' : 'mx-auto'}`} />
              {isSidebarOpen && <span className="ms-3">Pendataan Barang</span>}
            </NavLink>
            <NavLink to="/generate-laporan" className={({ isActive }) => `flex items-center py-4 m-2 px-2 rounded-sm ${isActive ? 'bg-[#718ADE]' : 'hover-side'}`}>
            ...
            <NavLink to="/buka-tutup-lelang" className={({ isActive }) => `flex items-center py-4 m-2 px-2 rounded-sm ${isActive ? 'bg-[#718ADE]' : 'hover-side'}`}>
              <Gavel
      ...
      {isMobile && (
        <div className="fixed bottom-0 left-0 z-50 w-full h-16 bg-[#4264D0] border-t border-gray-200  flex justify-around items-center">
          <NavLink 
            to="/dasboard-admin" 
            className={({ isActive }) => `flex flex-col items-center text-white p-2 rounded-sm ${isActive ? 'text-blue-600 bg-[#718ADE]' : 'hover-side'}`}
          >
